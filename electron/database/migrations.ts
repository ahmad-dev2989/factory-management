  ...
  return new Promise((resolve, reject) => {
    ...
    db.get('PRAGMA user_version', (err, row: any) => {
      ...
      if (currentVersion < 1) {
        ...
        // Execute migrations in a serialized sequence
        db.serialize(() => {
          ...
          db.exec(SCHEMA_V1, (execErr) => {
            ...
            db.run(`
              ...
              (4, 'InterGlobal Trading', 'John Doe', '+1-555-0199', '', 'info@interglobal.com', '100 Pine Street', 'Suite 2400', 'San Francisco', 'California', 'USA', '94111', '', 'International', 0, 0, 0, 'Inactive', 'Currently suspended due to inactive trade.')
            `);
            ...
            db.run('PRAGMA user_version = 1', (versionErr) => {
              if (versionErr) {
                console.error('[Database] Failed to update user_version, rolling back...', versionErr);
              ...
              db.run('COMMIT', (commitErr) => {
                if (commitErr) {
                  console.error('[Database] Failed to commit migration transaction:', commitErr);
                  reject(commitErr);
                } else {
        ...
        console.log('[Database] Database is already at version 1 (up to date).');
        resolve();
      }
    });
  });
