...
import { getDatabase, executeQuery } from '../database/database.js';

const logPath = path.join(app.getPath('userData'), 'app.log');

export function logMessage(level: 'INFO' | 'WARNING' | 'ERROR', moduleName: string, message: string) {
...
export async function exportLogs(window: BrowserWindow | null): Promise<boolean> {
  try {
...
export async function getDatabaseDiagnostics(): Promise<any> {
  try {
...
export function cleanupTemporaryStorage(): boolean {
  try {
