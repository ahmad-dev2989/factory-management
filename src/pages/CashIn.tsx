...
export default function CashIn() {
  ...
  // Data States
  const [vouchers, setVouchers] = useState<CashInItem[]>([]);
  ...
  // Sorting
  const [sortField, setSortField] = useState<keyof CashInItem>('voucherNumber');
  ...
  // Form State
  const [editingVoucher, setEditingVoucher] = useState<CashInItem | null>(null);
  ...
  // View State
  const [viewingVoucher, setViewingVoucher] = useState<CashInItem | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<CashInItem | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      // 3. Fetch Cash In Vouchers
      const vouchersRes = await (window as any).electron.invoke(
        'db-query',
        `SELECT ci.*, b.name AS account_name 
         FROM cash_in ci
      ...
      console.error('[CashIn] Failed to fetch data:', err);
    }
  };
  ...
  const generateVoucherNumber = async () => {
    try {
      const res = await (window as any).electron.invoke('db-query', 'SELECT MAX(id) AS max_id FROM cash_in');
      const nextId = (res && res[0] && res[0].max_id) ? (res[0].max_id + 1) : 1;
      const startingNo = 1001;
      return `CI-${startingNo + nextId - 1}`;
    } catch (err) {
  ...
  const handleEditClick = (v: CashInItem) => {
  ...
  const handleSaveVoucher = async (e: React.FormEvent) => {
    ...
    if (!receivedFrom.trim()) errors.receivedFrom = 'Received From is required';
    ...
    try {
      ...
      const checkDup = await (window as any).electron.invoke(
        'db-query',
        'SELECT id FROM cash_in WHERE LOWER(voucher_number) = ? AND id != ?',
      ...
      if (editingVoucher) {
        const updateRes = await (window as any).electron.invoke('cash-in-update', editingVoucher.id, payload);
        ...
        const createRes = await (window as any).electron.invoke('cash-in-create', payload);
        if (createRes && !createRes.error) {
          setSuccessMessage('Receipt recorded and account credited.');
          setIsVoucherModalOpen(false);
          fetchData();
          setTimeout(() => setSuccessMessage(''), 4000);
        } else {
      ...
      console.error('[CashIn] Save voucher failure:', err);
    }
  };

  const handleViewDetails = (v: CashInItem) => {
    setViewingVoucher(v);
    setIsViewModalOpen(true);
  };

  const handleDeleteClick = (v: CashInItem) => {
    if (v.status === 'Cancelled') {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const cancelRes = await (window as any).electron.invoke('cash-in-cancel', deleteTarget.id);
      if (cancelRes && !cancelRes.error) {
        setSuccessMessage(`Receipt ${deleteTarget.voucherNumber} cancelled. Bank balance debited.`);
      ...
      console.error('[CashIn] Cancellation failure:', err);
    }
  };

  const handlePrintClick = (v: CashInItem) => {
    setViewingVoucher(v);
    setTimeout(() => {
      window.print();
    }, 200);
  ...
  const handleRefresh = () => {
    fetchData();
    setSuccessMessage('Cash In vouchers list refreshed.');
        ...
        <div id="print-invoice" className="hidden print:block font-sans text-xs p-8 bg-white text-black leading-tight">
          <div className="flex items-start justify-between border-b pb-4 mb-4">
            ...
            <div className="text-right">
              <h2 className="text-lg font-black uppercase text-gray-600 tracking-wider">Cash Receipt Voucher</h2>
          ...
          <div className="grid grid-cols-2 gap-6 my-6 border-b pb-4">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-gray-400 uppercase">Received From:</span>
              <p className="text-sm font-bold text-gray-900">{viewingVoucher.receivedFrom}</p>
            </div>
          ...
          <div className="bg-gray-50 p-4 rounded border flex justify-between items-center my-6">
            <span className="text-sm font-extrabold text-gray-600 uppercase">Amount Received:</span>
            <span className="text-lg font-black text-green-700">{formatCurrency(viewingVoucher.amount)}</span>
          </div>
      ...
      <div className="bg-white border-b border-[#E5E7EB] py-5 px-8 flex flex-col justify-center print:hidden">
        <div className="max-w-[1500px] w-full mx-auto flex flex-col gap-1.5">
          <div className="flex items-center gap-2 text-xs font-semibold text-[#6B7280] tracking-wide uppercase">
            ...
            <span className="text-[#1F2937]">Cash In Manager</span>
          </div>

          <h2 className="text-xl font-bold text-[#1F2937] mt-1">Cash In (Vouchers)</h2>
          <p className="text-xs text-[#6B7280] font-semibold tracking-wide uppercase">
            Record, track, and manage incoming cash flow adjustments, sales receipts, and business investments.
          </p>
        </div>
      </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="relative max-w-sm w-full">
                  ...
                  <input
                    ...
                    placeholder="Search by Voucher, Ref, Received From..."
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  >
                    <Plus className="w-4 h-4" /> New Cash In
                  </button>
                  <button
                    type="button"
            ...
            <div className="overflow-x-auto border border-[#E5E7EB] rounded-[8px]">
              {paginatedVouchers.length > 0 ? (
                <table className="min-w-full divide-y divide-[#E5E7EB] text-left text-sm whitespace-nowrap">
                  <thead className="bg-[#F6F8FB] text-[#6B7280] font-semibold text-xs uppercase tracking-wider select-none">
                    <tr>
                      ...
                      {visibleColumns.includes('receivedFrom') && (
                        <th
                          className="px-4 py-3.5 cursor-pointer hover:bg-gray-100/50"
                          onClick={() => {
                            setSortField('receivedFrom');
                            setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc');
                          }}
                        >
                          <div className="flex items-center gap-1">
                            Received From
                            {sortField === 'receivedFrom' && (sortOrder === 'asc' ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />)}
                          </div>
                        </th>
                      )}
                      ...
                      {visibleColumns.includes('accountName') && (
                        <th className="px-4 py-3.5">Deposited Account</th>
                      )}
                    ...
                    {paginatedVouchers.map((v) => {
                      ...
                      return (
                        <tr key={v.id} className={`hover:bg-[#F6F8FB]/50 transition-colors ${v.status === 'Cancelled' ? 'opacity-65' : ''}`}>
                          ...
                          {visibleColumns.includes('receivedFrom') && (
                            <td className="px-4 py-4 font-bold text-gray-800">{v.receivedFrom}</td>
                          )}
                          {visibleColumns.includes('category') && (
                          ...
                          {visibleColumns.includes('amount') && (
                            <td className="px-4 py-4 text-right font-bold text-[#27AE60]">{formatCurrency(v.amount)}</td>
                          )}
                ...
                <div className="p-12 text-center text-[#6B7280] text-sm font-semibold select-none bg-white">
                  No Cash In vouchers found. Click 'New Cash In' to record inflow.
                </div>
              )}
            </div>
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-[10px] shadow-lg max-w-lg w-full p-6 flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3 shrink-0">
              <h3 className="text-base font-bold text-[#1F2937]">
                {editingVoucher ? 'Edit Cash In Voucher' : 'Record Cash Receipt'}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveVoucher} className="flex-grow overflow-y-auto py-5 space-y-4 text-xs font-semibold text-gray-700">
              ...
              {/* Received From */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] uppercase font-bold text-gray-500">Received From *</label>
                <input
                  type="text"
                  value={receivedFrom}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. Slip #4321 / Bank Transfer Ref"
              ...
              {/* Bank Account */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] uppercase font-bold text-gray-500">Deposited Account *</label>
                <select
            ...
            <div className="flex-grow overflow-y-auto py-5 space-y-4 text-xs font-semibold text-gray-700">
              ...
              <div className="space-y-3.5 border-t border-b py-3.5">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-800 font-bold">{viewingVoucher.receivedFrom}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Category</span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-400">Deposited Account</span>
                ...
                <div className="flex justify-between items-center bg-green-50 p-2.5 rounded text-green-800">
                  <span className="font-extrabold uppercase">Amount Credited</span>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-[#E5E7EB] rounded-[10px] shadow-lg max-w-md w-full p-6 space-y-4 animate-fadeIn">
            <div className="flex items-center gap-3 text-red-500">
              ...
              <h3 className="text-base font-bold text-[#1F2937]">Cancel Cash In Voucher?</h3>
            </div>

            <p className="text-sm text-gray-600 font-medium leading-relaxed">
              Are you sure you want to cancel Cash In Voucher <strong>{deleteTarget.voucherNumber}</strong>?
              This operation will soft-delete the receipt by setting its status to <strong>Cancelled</strong>.
              <br />
              <span className="text-xs text-red-500 font-bold">
                * The corresponding bank account balance will be automatically debited.
              </span>
            </p>
